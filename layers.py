...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean * weight
        running_var = momentum * running_var + (1 - momentum) * sample_var * weight
        ...
        cache = x, gamma, beta, running_mean, running_var, eps, x_hat
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
...
def batchnorm_backward(dout, cache):
    ...
    x, gamma, beta, running_mean, running_var, eps,x_hat = cache
    ...
    m = x.shape[0]
    dx_hat = dout * gamma
    dsigma = np.sum(dx_hat * (x - running_mean) * -0.5 * (running_var + eps)**(-3/2), axis = 0)
    dmu = np.sum (dx_hat * (-1.0 / np.sqrt(running_var + eps)), axis=0) + (dsigma * np.sum(((-2.0)* (x - running_mean) / m), axis = 0))
    dx = dx_hat / np.sqrt(running_var + eps) + (dsigma * 2.0 *(x - running_mean) / m) + (dmu / m)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    ###########################################################################
    # __padding__
    x_p = np.zeros((N,H+(pad*2),W+(pad*2)))
    for i in range(N):
        x_p[i] = np.pad(x[i],pad,'constant',constant_values=0)
                ...
                for c in range(W1):
                    ...
                    for xi in range(HH):
                        for yi in range(WW):
                            f = w[j]
                            m = x_p[i]
                            out[i][j][r][c]+= f[xi][yi]*m[r*stride+xi][c*stride+yi]
    ...
    for i in range(N):
        for j in range(F):
            ...
            for r in range(H1):
                for c in range(W1):
                    d = dout[i][j][r][c]
                    xi_start = r*stride-pad
                    yi_start = c*stride-pad
                    for xi in range(HH):
                        for yi in range(WW):
                            if xi_start+xi>=0 and yi_start+yi>=0 and xi_start+xi<H and yi_start+yi<W:
                                dx[i][xi_start+xi][yi_start+yi] += d*w[j][xi][yi]
                                dw[j][xi][yi]+= d*x[i][xi_start+xi][yi_start+yi]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for j in range(C):
            for r in range(H1):
                for c in range(W1):
                    out[i][j][r][c] = x[i][j][r*stride][c*stride]
                    xm = 0
                    ym = 0
                    for xi in range(h):
                        for yi in range(w):
                            if x[i][j][r*stride+xi][c*stride+yi]>out[i][j][r][c]:
                                out[i][j][r][c] = x[i][j][r*stride+xi][c*stride+yi]
                                xm = xi
                                ym = yi
                    mask[i][j][r*stride+xm][c*stride+ym] = 1
        ...
        for j in range(C):
            for r in range(H1):
                for c in range(W1):
                    for xi in range(h):
                        for yi in range(w):
                            dx[i][j][r*stride+xi][c*stride+yi]*=dout[i][j][r][c]
